...
namespace FinanzasPersonales.NET.Views
{
    ...
    public class InversionView
    {
        ...
        {
            Console.Clear();
            EstiloApp.MostrarSubtitulo("=== ACTUALIZAR VALORES ===");
            ...
            EstiloApp.MostrarInfo("Actualizando todas las inversiones...");

            int actualizadas = 0;
            foreach (var inversion in inversiones)
            {
                if (await _controller.ActualizarValorInversionAsync(inversion.Id))
                {
                    actualizadas++;
                }
            }

            EstiloApp.MostrarExito($"{actualizadas} inversiones actualizadas");
            EstiloApp.EsperarTecla();
        }
